...
from huds_app.config import inspect_config as _inspect_loaded_config
...
from huds_app.train import apply_normalization, load_normalization
...
def _remaining_unlabeled_count(train_pool_df: pd.DataFrame | None, labeled_ids: set[Any], state: RunState) -> int:
    if train_pool_df is None or SAMPLE_ID_COLUMN not in train_pool_df.columns:
        return 0
    # FIX 1: Only block IDs that are truly used (in training datasets), not pending ones
    blocked_ids = set(state.used_sample_ids) | labeled_ids
...
def _next_command(
    ...
    FIX 1: Use proper state tracking via train_requests status instead of pending_sample_ids.
    """
    ...
    # Check if current step's training data is labeled
    current_step_key = str(state.current_step)
    train_status = state.train_requests.get(current_step_key, {}).get("status", "")
    ...
    has_pending_train = any(
        req.get("status") == "exported" or req.get("status") == "partial" 
        for req in state.train_requests.values()
    )
